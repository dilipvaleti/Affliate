...
def affliate_flipkart():
    ...
    # Removes the html tags available in the response of url_short call
    def cleanhtml(raw_html):
      cleanr = re.compile('<.*?>')
      cleantext = re.sub(cleanr, '', raw_html)
      return cleantext

    # Shorten the long url by using https://rapidapi.com/stefanluciandeleanu/api/shorturl-sfy-cx api
    def url_shorten(original_url):
        url = "https://shorturl-sfy-cx.p.rapidapi.com/"
        querystring = {"url": original_url}
        headers = {
            'x-rapidapi-host': "shorturl-sfy-cx.p.rapidapi.com",
            'x-rapidapi-key': "627340e2e9msh494217d003bfe31p12c61djsnfd9b5c6948cc"
        }
        time.sleep(2)
    ...
    while True:
        #get the today offers from the flipkart by calling relevant API
        category_name_list=flipkart_api.load_products_dict('https://affiliate-api.flipkart.net/affiliate/offers/v1/all/json')
        prod_dic={}
        for prod in category_name_list['allOffersList']:
        #     print(prod)
        #     print(prod['url'])
        #     print(url_shorten(prod['url']))

            prod_dic[str(prod['name'])+' -- '+str(prod['description'])+' -- '+str(prod['title'])] = url_shorten(prod['url'])
        #     print(prod_dic)

        #send a message to the telegram for each product received in the above.
        for product in prod_dic.keys():
            #html=produc
            text_data=product+' '+prod_dic[product]
            text_data=text_data.replace('&','and')
